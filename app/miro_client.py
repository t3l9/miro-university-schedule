...
# ============================================================
# Чтение элементов с доски — актуальные позиции объектов
# (нужно, чтобы определить, в какой ячейке сейчас карточка)
# ============================================================
...
def list_items(item_type: Optional[str] = None,
    ...
    """
    Вернуть ВСЕ элементы доски (с автоматической постраничной подгрузкой).

    item_type — необязательный фильтр Miro ('shape', 'text', 'frame', ...).
    Miro отдаёт максимум 50 за страницу и курсор для следующей.
    """
    items: list[dict] = []
    ...
    while True:
        path = f"/items?limit={limit}"
        if item_type:
            path += f"&type={item_type}"
        if cursor:
            path += f"&cursor={cursor}"
        data = _request("GET", path, board_id=board_id)
...
def get_item(item_id: str, board_id: Optional[str] = None) -> dict:
    """
    Прочитать один элемент по ID через общий items-эндпоинт
    (работает для любого типа: shape/text/frame).
    Возвращает в т.ч. актуальные position и geometry.
    """
    return _request("GET", f"/items/{item_id}", board_id=board_id)


# ============================================================
# GROUP — склеиваем элементы карточки в одну группу,
# чтобы Miro двигал их вместе при перетаскивании.
# ============================================================

def create_group(item_ids: list[str], board_id: Optional[str] = None) -> Optional[dict]:
    """
    Сгруппировать элементы карточки. Возвращает объект группы либо None.

    Best-effort: при любой ошибке группировки молча возвращаем None — карточка
    остаётся набором отдельных объектов (рабочим, просто перетаскивается
    поэлементно). Группировка не должна ронять создание карточки.
    """
    item_ids = [i for i in item_ids if i]
    if len(item_ids) < 2:
        return None
    try:
        return _request("POST", "/groups", board_id=board_id,
                        json={"data": {"items": item_ids}})
    except MiroError:
        return None