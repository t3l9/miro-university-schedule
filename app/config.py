...
# --- Чётность недели (классический «числитель / знаменатель») ---
# Каждую ячейку (день × слот) можно занять либо одной парой «на каждую неделю»
# (она занимает ячейку целиком), либо двумя половинами:
#   нечётная неделя (числитель, «I») — сверху,
#   чётная неделя   (знаменатель, «II») — снизу.
WEEK_BOTH = "обе"        # пара идёт каждую неделю — на всю ячейку
WEEK_ODD = "нечётная"    # числитель,  неделя I  — верхняя половина ячейки
WEEK_EVEN = "чётная"     # знаменатель, неделя II — нижняя половина ячейки

# Порядок важен: первым идёт значение по умолчанию.
WEEKS = [WEEK_BOTH, WEEK_ODD, WEEK_EVEN]

# Человекочитаемые подписи (для /meta и Swagger).
WEEK_LABELS = {
    WEEK_BOTH: "Каждую неделю",
    WEEK_ODD: "I · нечётная (числитель)",
    WEEK_EVEN: "II · чётная (знаменатель)",
}

# Короткий бейдж, который рисуется в углу половинной карточки.
WEEK_BADGE = {
    WEEK_BOTH: "",
    WEEK_ODD: "I",
    WEEK_EVEN: "II",
...
# Цвет уголка-бейджа недели (нейтральный, чтобы не спорить с цветом типа пары).
WEEK_BADGE_BG = {
    WEEK_BOTH: "#64748B",  # для «обе» бейдж не рисуется
    WEEK_ODD: "#0EA5E9",   # sky-500 — числитель
    WEEK_EVEN: "#8B5CF6",  # violet-500 — знаменатель
}
...
# --- Деление ячейки на числитель/знаменатель ---
# Когда в одной ячейке две пары (нечётная сверху, чётная снизу), между ними
# оставляем зазор и рисуем тонкий пунктир-разделитель.
SUBCELL_GAP = 16          # вертикальный зазор между верхней и нижней половинами
SUBCELL_DIVIDER_H = 3     # толщина линии-разделителя
COLOR_SUBCELL_DIVIDER = "#CBD5E1"  # slate-300 — мягкая серая линия
