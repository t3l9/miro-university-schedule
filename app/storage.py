...
def find_lesson_by_slot(day: str, slot: str) -> Optional[dict]:
    """
    Найти ЛЮБУЮ пару по (день, слот) без учёта чётности.
    Оставлено для обратной совместимости. Возвращает первую найденную либо None.
    Для логики с чётностью используйте find_lessons_in_cell / find_conflict.
    """
...
def find_lessons_in_cell(day: str, slot: str) -> list[dict]:
    """Все пары в ячейке (день, слот) — их может быть до двух (числитель + знаменатель)."""
    return [
        lesson for lesson in load()["lessons"].values()
        if lesson["day"] == day and lesson["slot"] == slot
    ]


# Что с чем конфликтует по чётности:
#   "обе" занимает всю ячейку и не уживается ни с чем;
#   "нечётная" конфликтует только с "нечётной" (и с "обе");
#   "чётная"   конфликтует только с "чётной"   (и с "обе").
def _weeks_conflict(a: str, b: str) -> bool:
    if a == "обе" or b == "обе":
        return True
    return a == b


def find_conflict(day: str, slot: str, week: str,
                  exclude_id: Optional[str] = None) -> Optional[dict]:
    """
    Вернуть пару, которая помешает поставить новую пару с данной чётностью
    в ячейку (день, слот), либо None, если место свободно.

    exclude_id — id пары, которую при проверке игнорируем (нужно при move/patch
    самой себя).
    """
    for lesson in find_lessons_in_cell(day, slot):
        if exclude_id is not None and lesson["id"] == exclude_id:
            continue
        if _weeks_conflict(lesson.get("week", "обе"), week):
            return lesson
    return None
